...
File Cooker 现在支持在以下右键菜单中直接触发：

- **编辑器右键菜单**
  - `File Cooker > Selection`：同步选区到 flomo / 添加选区到 canvas（仅在有选区时可用）
  - `File Cooker > Current file links`：对当前文件中的链接执行批量操作
- **文件管理器右键菜单**
  - `File Cooker > Target file`：对「被右键的文件」执行单文件操作

说明：
- 删除等危险操作仍会弹出确认窗口。
- 当不存在活动文件时，当前文件链接相关操作会被安全阻断并提示。
